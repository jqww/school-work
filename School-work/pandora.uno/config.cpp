...
void Config::save( std::string name, bool to_clipboard ) {
	...
	for( auto& e : g_cfg ) {
		WritePrivateProfileStringA( XOR( "cantresolve" ), e.first.c_str( ), std::to_string( e.second.get<double>( ) ).c_str( ), file.c_str( ) );
	}
...
void Config::load( std::string name, bool from_clipboard ) {
	...
	for( auto& e : g_cfg ) {
		...
		GetPrivateProfileStringA( XOR( "cantresolve" ), e.first.c_str( ), "", value, 64, file.c_str( ) );
