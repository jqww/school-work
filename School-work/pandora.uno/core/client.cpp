...
void Client::DrawHUD( ) {
	...
	std::string text = tfm::format( XOR( "cantresolve.us by moontiz" ) );
...
void Client::ClanTag( )
{
	...
	auto SetClanTag = [ & ] ( std::string tag ) -> void {
		...
		SetClanTagFn( tag.c_str( ), XOR( "cantresolve.us" ) );
	};

	std::string szClanTag = XOR( "cantresolve.us " );
...
void Client::print( const std::string text, ... ) {
	...
	// print to console.
	g_csgo.m_cvar->ConsoleColorPrintf( clr, XOR( "[ cantresolve ] " ) );
